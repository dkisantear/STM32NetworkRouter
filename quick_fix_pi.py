#!/usr/bin/env python3
"""
Quick fix script - Copy this entire file to replace pi_stm32_bridge.py on your Pi
This version:
- Uses correct device ID: stm32-master
- Doesn't timeout when no messages are received initially
- Sends heartbeats every 30 seconds to keep status online
"""
...
def main():
    ...
    ser = None
    last_message_time = None  # Only set when we receive actual messages
        ...
        try:
            ...
            if ser is None or not ser.is_open:
                try:
                    ...
                    # Don't initialize last_message_time here - only set when we receive messages
                ...
                try:
                    ...
                    if ser.in_waiting > 0:
                        ...
                        if line:
                            ...
                            # Update last_message_time when we receive ANY message
                    ...
                    # Check for timeout - only if we've received messages before
                    # If Master board isn't sending data yet, rely on heartbeat to keep status online
